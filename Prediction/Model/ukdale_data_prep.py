...
chosen_start_date = '2015-02-03'
chosen_end_date = '2016-02-02'

house_num = 'UK_dale_house_1'
...
def get_data(path = '/Users/nelson/Academia/Research/Work/SmartHEMS/Prediction/Data Source/ukdale/House1'):
    ...
    for file in os.listdir(path):
        ...
        if file == 'Gas_oven_House_1.dat':
            power_threshold = 10
        if file == 'Vacumm_cleaner_House_1.dat':
            power_threshold = 5
        if file == 'Hair_straightner_House_1.dat':
            power_threshold = 5
        if file == 'Home_theater_pc_House_1.dat':
            power_threshold = 10
        if file == 'Kitchen_light_House_1.dat':
            power_threshold = 10
        if file == 'Subwoofer_House_1.dat':
            power_threshold = 10
        if file == 'Television_House_1.dat':
            power_threshold = 10
        if file == 'Dvd_player_House_4.dat':
            power_threshold = 25
        ...
        with open(file_path, "r") as f:
            datas = f.readlines()
        # print(datas[0:10])
            # for line in tqdm(f.readlines(), desc = "Reading power data from %s"%file):
        ...
        for row in tqdm(datas, desc = "Reading power data from %s"%file):
            # print(row)
            time, power = row.split()

            if power == 'null' or power == None:
                power = 0
            else:
                power = float(power)
            # print('Power ', power)

            dttime = datetime.datetime.fromtimestamp(int(time))
            ...
            if prev_time == None:
                ...
                time_diff = round(datetime.timedelta.total_seconds(dttime - starttime)/60)
                for i in range(1, time_diff):
                    miss_time = starttime + datetime.timedelta(minutes = i)
...
#fill the gap between last reading and end time
        if prev_time < endtime:
            # print(prev_time)
            time_diff = round(datetime.timedelta.total_seconds(endtime - prev_time)/60)
            if time_diff >= 1:
                for i in range(1, time_diff):
                    miss_time = prev_time + datetime.timedelta(minutes = i)
                    days = datetime.datetime.weekday(miss_time)
...
def get_selected_data(data):
    ...
    for device in data:
        ...
        # print('last processing time: ', time)
        count = 0
    ...
    array_lst = []
    for item in array_select:
        lst = []
        for items in item:
            lst.append(len(items))
        array_lst.append(len(lst))
    print(array_lst)
...
# select_data_1 format: [[operations of 24 hours in a day], day_info]
    ...
    foupath = os.path.join(PATH, '%s_dataset.pkl'%house_num)
    ...
    PATH = '/Users/nelson/Academia/Research/Work/SmartHEMS/Prediction/Data Source/pre-process'    
    foupath = os.path.join(PATH, '%s_distribution.pkl'%house_num)
    with open(foupath, 'wb') as f:
...
# select_data_2: how many hours per day in operation
    PATH = '/Users/nelson/Academia/Research/Work/SmartHEMS/Prediction/Data Source/pre-process'    
    foupath = os.path.join(PATH, '%s_day_op_count.pkl'%house_num)
    with open(foupath, 'wb') as f:
        # print(np.array(selected_data_1).shape)
    ...
    PATH = '/Users/nelson/Academia/Research/Work/SmartHEMS/Prediction/Data Source/pre-process'    
    foupath = os.path.join(PATH, '%s_day_info.pkl'%house_num)
    with open(foupath, 'wb') as f:
        # print(np.array(selected_data_1).shape)
...
# calculate hourly power total for energy saving calculation
    ...
    PATH = '/Users/nelson/Academia/Research/Work/SmartHEMS/Prediction/Data Source/pre-process'    
    foupath = os.path.join(PATH, '%s_power.pkl'%house_num)
    with open(foupath, 'wb') as f:
        # print(np.array(selected_data_1).shape)
...
def training_testing_index(data):
    ...
    PATH = '/Users/nelson/Academia/Research/Work/SmartHEMS/Prediction/Data Source/pre-process'    
    foupath = os.path.join(PATH, '%s_indexing.pkl'%house_num)
    with open(foupath, 'wb') as f:
        pickle.dump(index, f)
...
if __name__ == "__main__": 
    ...
    print(np.array(state_data).shape)
    print(dev_list)
    ...
    print('Daily op shape: ', daily_op_sum.shape)
